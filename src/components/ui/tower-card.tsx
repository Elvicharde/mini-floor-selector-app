...
const TowerCard: React.FC<Tower> = ({
  ...
  return (
    <CustomCardWrapper>
      <Card className="group/tower-card overflow-hidden shadow-md transition-transform hover:scale-[1.02] cursor-pointer p-0 sm:flex-row bg-inherit border border-xl border-white">
        ...
        <div className="relative h-80 sm:w-[40%] overflow-hidden">
          <img
            ...
            alt={`${title} tower`}
            className={`absolute inset-0 h-full w-full object-cover transition-transform duration-300 ease-in-out group-hover/tower-card:scale-110 ${
              isActive ? "scale-110" : ""
            }`}
            {...handlers}
          />
