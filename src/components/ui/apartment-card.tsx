...
import CustomCardWrapper from "../shared/custom-card-wrapper";
...
const ApartmentCard: React.FC<ApartmentLayout> = ({
  ...
  return (
    <CustomCardWrapper>
      <Card className="group/apartment-card overflow-hidden shadow-md transition-transform hover:scale-[1.02] cursor-pointer p-0 bg-white border border-xl border-white h-full">
        {/* Image wrapper keeps dimensions consistent */}
        <div className="relative overflow-hidden h-64">
          <img
            src={thumbnail ?? "/assets/apartment-fallback.svg"}
            alt={`floor ${apartmentNumber}`}
            className={`absolute inset-0 h-full w-full object-cover transition-transform duration-300 ease-in-out group-hover/apartment-card:scale-110 ${
              isActive ? "scale-110" : ""
            }`}
            {...handlers}
          />
