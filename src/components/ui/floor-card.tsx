...
const FloorCard: React.FC<FloorLayout> = ({
  ...
  return (
    <CustomCardWrapper>
      <Card className="group/floor-card overflow-hidden shadow-md transition-transform hover:scale-[1.02] cursor-pointer p-0 bg-white border border-xl border-white h-full">
        {/* Image wrapper keeps dimensions consistent */}
        <div className="relative overflow-hidden h-64">
          <img
            src={thumbnail ?? "/assets/floor-fallback.svg"}
            alt={`floor ${floorNumber}`}
            className={`absolute inset-0 h-full w-full object-cover transition-transform duration-300 ease-in-out group-hover/floor-card:scale-110 ${
              isActive ? "scale-110" : ""
            }`}
            {...handlers}
          />
